...
def create_payment_request(list_of_docs=None):
    ...
    if list_of_docs:
        for invoice in list_of_docs:
            ...
            # Create Payment Request without any conditions
            ...
            # Directly setting Net Payable amount
            doc.grand_total = invoice_doc.custom_net_payable
            
            doc.save(ignore_permissions=True)
            
            frappe.db.set_value('Bulk Transaction Log Table', {
                'parent': update_dict[invoice],
                'parentfield': "bulk_transaction_log_table",
                'reference_doctype': 'Sales Invoice',
                'reference_name': invoice
            }, 'status', 'Completed')
